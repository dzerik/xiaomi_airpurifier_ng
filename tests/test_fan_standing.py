...
class TestNaturalMode:
    ...
    @pytest.mark.asyncio
    async def test_natural_mode_on_standard(self):
        """Standard fan uses set_natural_speed."""
        coord = _make_coordinator(model="zhimi.fan.v2", data={"speed": 50})
        fan = XiaomiStandingFan(coord)
        fan.hass = coord.hass
        await fan.async_set_natural_mode_on()
        coord.device.set_natural_speed.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_natural_mode_off_standard(self):
        """Standard fan uses set_direct_speed."""
        coord = _make_coordinator(model="zhimi.fan.v2", data={"speed": 50})
        fan = XiaomiStandingFan(coord)
        fan.hass = coord.hass
        await fan.async_set_natural_mode_off()
...
class TestDelayOff:
    """Tests for async_set_delay_off."""

    @pytest.mark.asyncio
    async def test_delay_off_p5_minutes(self):
        """P5-style passes minutes directly."""
        coord = _make_coordinator(model=MODEL_FAN_P5)
        fan = XiaomiStandingFan(coord)
        fan.hass = coord.hass
        await fan.async_set_delay_off(60)
        coord.device.delay_off.assert_called_once_with(60)

    @pytest.mark.asyncio
    async def test_delay_off_standard_seconds(self):
        """Standard fan converts minutes to seconds."""
        coord = _make_coordinator(model="zhimi.fan.v2")
        fan = XiaomiStandingFan(coord)
        fan.hass = coord.hass
        await fan.async_set_delay_off(60)
        coord.device.delay_off.assert_called_once_with(3600)
